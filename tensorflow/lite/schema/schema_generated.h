...
inline void RedundantData::UnPackTo(RedundantDataT *_o, const flatbuffers::resolver_function_t *_resolver) const {
  (void)_o;
  (void)_resolver;
  { auto _e = first_copy(); if (_e) { _o->first_copy.resize(_e->size()); for (flatbuffers::uoffset_t _i = 0; _i < _e->size(); _i++) { if(_o->first_copy[_i]) { _e->Get(_i)->UnPackTo(_o->first_copy[_i].get(), _resolver); } else { _o->first_copy[_i] = std::unique_ptr<tflite::BufferT>(_e->Get(_i)->UnPack(_resolver)); }; } } }
  { auto _e = second_copy(); if (_e) { _o->second_copy.resize(_e->size()); for (flatbuffers::uoffset_t _i = 0; _i < _e->size(); _i++) { if(_o->second_copy[_i]) { _e->Get(_i)->UnPackTo(_o->second_copy[_i].get(), _resolver); } else { _o->second_copy[_i] = std::unique_ptr<tflite::BufferT>(_e->Get(_i)->UnPack(_resolver)); }; } } }
}
...
inline void Model::UnPackTo(ModelT *_o, const flatbuffers::resolver_function_t *_resolver) const {
  ...
  { auto _e = redundancy(); if (_e) { if(_o->redundancy) { _e->UnPackTo(_o->redundancy.get(), _resolver); } else { _o->redundancy = std::unique_ptr<tflite::RedundantDataT>(_e->UnPack(_resolver)); } } }
}
