...
{
  static char cp[64];
  strncpy(cp, test_sample_name, sizeof("000000078266_0"));
  char *p = strtok(cp, "_");
  p = strtok(NULL, "_");
  return stoi(std::string(p));
}

bool RespondToDetection(bool is_preson_predicted, const char *filename) {
  return extract_ground_truth(filename) == is_preson_predicted;
}
